...
model =  h2o.deeplearning(x = 2:785,  y = 1,  training_frame = train_h2o, activation = "RectifierWithDropout", input_dropout_ratio = 0.2, hidden_dropout_ratios = c(0.5,0.5), balance_classes = TRUE, hidden = c(500,500),  momentum_stable = 0.99, nesterov_accelerated_gradient = T, epochs = 800) 
...
sub_1 = data.frame(ImageId = seq(1,length(sub$predict)), Label = sub$predict)
