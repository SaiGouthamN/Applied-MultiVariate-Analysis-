...
#run a support vector machine model with kernel -linear and cost  = 3.
svm_model <- svm(label~., data = train.data, 
                cost = 3, 
                kernel = "linear") 

#make prediction on test data
svm.prediction <- predict(svm_model,test.data)

#Submit the values to the output file
Sub_1<- data.frame(ImageId=seq(1,28000),label=svm.prediction)
