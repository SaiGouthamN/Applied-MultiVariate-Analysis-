...
#Assign 1 to the Sex field if male or 0 if female for test and train data
...
#Apply the generalized linear model to predcit the dependent variable Survived
train.glm <- glm(Survived ~  Pclass  + Age*Sex + Sex*Pclass+Sex +Family,family = binomial, data = trainData)
p.hats <- predict.glm(train.glm, newdata = testData, type = "response")


#Convert the predicted to either 1 or 0.
survival <- vector()
...
write.csv(kaggle.sub, file = "C:\\Users\\sai-g\\Desktop\\Goutham\\3.App.Multivariate Analysis\\Project\\kaggle_glm_2.csv", row.names = FALSE)
