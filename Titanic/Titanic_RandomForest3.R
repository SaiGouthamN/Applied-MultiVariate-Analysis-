...
#Removing unwanted columns from testData
...
#Assign 1 to Sex if it is male or 0 if female 
...
#Applyting the random forest model to predic the independent varaible
my_forest <- randomForest(as.factor(Survived) ~   Pclass*Age + Sex + SibSp*Sex + Parch*Family + Family ,
                          data = trainData, importance = TRUE,na.action=na.omit, ntree = 1000)
...
write.csv(kaggle.sub, file = "C:\\Users\\sai-g\\Desktop\\Goutham\\3.App.Multivariate Analysis\\Project\\Kaggle_RForest_2.csv",row.names = FALSE)
