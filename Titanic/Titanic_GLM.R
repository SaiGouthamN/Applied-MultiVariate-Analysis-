...
for(i in dr_vector) {
  ...
  for (i in 1:nrow(trainData)) {
    if (is.na(trainData[i,5])) {
      ...
      train.glm <- glm(Survived ~ Pclass  + Age*Sex + Sex*Pclass+Sex+Family+Mother, family = binomial, data = trainData)
...
write.csv(kaggle.sub, file = "C:\\Users\\sai-g\\Desktop\\Goutham\\3.App.Multivariate Analysis\\Project\\kaggle.csv", row.names = FALSE)