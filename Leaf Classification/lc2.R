...
qqline(qqnorm(leaf_train$shape27))
...
test.data<-data.frame(Species=leaf_test$id,leaf_pca_margin1$x,leaf_pca_shape1$x,leaf_pca_texture1$x,row=leaf_test$ROW_PIXEL,col=leaf_test$COL_PIXEL,size=leaf_test$s)

names(test.data)
names(train.data)

#we are interested in first 30 PCAs
train.data <- train.data[, c(1:2,65,129:131,194:196)]
test.data<-test.data[, c(1:2,65,129:131,194:196)] 




#run a decision tree
rpart.model <- randomForest(Species ~ .,data = train.data,method='class',ntree=61000)
...
write.csv(Sub_1,'C:\\Users\\sai-g\\Desktop\\Goutham\\3.App.Multivariate Analysis\\Project\\Leaf Classification\\file_sub3.csv',row.names=FALSE)
